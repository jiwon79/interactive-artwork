  ...
  public updatePixelMatrix() {
    ...
    for (let i = 0; i < Constant.THETA_NUM; i++) {
      for (let j = 0; j < Constant.PHI_NUM; j++) {
        ...
        const r: Vector = this.solidService.getRotatedRVector(parameter, this.rotate);
        const normal = this.solidService.getRotatedNormalVector(parameter, this.rotate);

        const luminance = this.solidService.getLuminance(r, normal, PixelService.LIGHT);
