...
export type ColorShaderType = 'grey' | 'rainbow' | 'red-gradation' | 'change-rainbow';
...
function getRainbowColor(parameter: Parameter): [number, number, number] {
  ...
  const g = Math.floor(255 * (Math.sin(parameter.phi + 2 * pi / 3) * 0.5 + 0.5));
  const b = Math.floor(255 * (Math.sin(parameter.theta + 4 * pi / 3) * 0.5 + 0.5));
...
function getChangeRainbowColor(parameter: Parameter, time: number): [number, number, number] {
  const pi = Math.PI;
